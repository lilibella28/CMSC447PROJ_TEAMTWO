...
class registry:
    ...
    * :meth:`_orm.registry.map_imperatively` will produce a
    ...
    _number_of_dependents: Set[_RegistryType]
    ...
    def __init__(
        ...
        *,
        ...
        self._number_of_dependents = set()
    ...
    def _set_depends_on(self, registry: RegistryType) -> None:
        if registry is self:
            return
        registry._number_of_dependents.add(self)
    ...
    def _flag_new_mapper(self, mapper: Mapper[Any]) -> None:
        ...
        for reg in self._recurse_with_number_of_dependents({self}):
            reg._new_mappers = True

    @classmethod
    def _recurse_with_number_of_dependents(
        ...
        while todo:
            ...
            # if yielding would remove number_of_dependents, make sure we have
            # them before
            todo.update(reg._number_of_dependents.difference(done))
            yield reg

            # if yielding would add number_of_dependents, make sure we have them
            # after
            todo.update(reg._number_of_dependents.difference(done))
